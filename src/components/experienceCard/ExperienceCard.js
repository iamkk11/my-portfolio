...
export default function ExperienceCard({ cardInfo }) {
  ...
  const GetDescBullets = ({ descBullets }) => {
    return descBullets ? descBullets.map((item) => <li className="experience-text-desc">{item}</li>) : null
  };

  return (
    <div className="experience-card">

      <a href={cardInfo.url} target="_blank" rel="noopener noreferrer" style={{background: rgb(colorArrays) }} className="experience-banner">
